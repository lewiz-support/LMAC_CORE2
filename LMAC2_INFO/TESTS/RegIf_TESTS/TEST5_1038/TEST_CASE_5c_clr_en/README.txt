...
As the packets are received the FMAX_RX_BYTE_CNT_LO register accumulated the number of bytes received
Clear signal is provided from the axi_master by asserting the 7th bit of the fmac_ctrl signal from the axi_master
Once the clear signal is provided, the FMAX_RX_BYTE_CNT_LO changes its value to 0 and accordingly the mac_regdout signal in the axi_master
