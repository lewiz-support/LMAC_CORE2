...
/* DMA TX/RX Base offsets. M2S = memory to stream */
#define DMA_M2S_OFFSET		0x4000
#define DMA_S2M_OFFSET		0x5000

//from zynqmp LMAC2 demo
...
#define DMA_R_CR		0x0		//control reg
#define DMA_R_SR		0x4		//stat reg
#define DMA_R_ADDR		0x18		//addr reg
...
/*
 * Prepare the RX path. Set DMA ready to receive 1 pkt from LMAC
 */
static void lmac_prepare_rx(struct net_device *dev)
{
	...
	lp->rx.phys = dma_map_single(dev->dev.parent,	//return rx.phys DMA address
			lp->rx.skb->data, MAX_FRAME_SIZE,		//virt addr
			DMA_FROM_DEVICE);

	/* Program the 64bit DMA address registers.  */
...
//when pkt come in from LMAC, DMA auto pump into memory
//then set interrupt flag
//which invokes this interrupt handler
static irqreturn_t lmac2_rx_interrupt(int irq, void *dev_id)
{
	...
	/* read to Check the DMA Status Register for pending RX interrupts.  */
...
static irqreturn_t lmac2_tx_interrupt(int irq, void *dev_id)
{
	...
	u32 tx_sr;
        ...
        //read stat reg, for interrupt bit
	tx_sr = readl(lp->base_addr + DMA_M2S_OFFSET + DMA_R_SR);

	//take care of the interrupt
	if (tx_sr & DMA_SR_IOC_IRQ) {
		// Ack the IRQ.  by write to the status reg
		writel(DMA_SR_IOC_IRQ,
			lp->base_addr + DMA_M2S_OFFSET + DMA_R_SR);
		dma_unmap_single(dev->dev.parent, lp->tx.phys,
				lp->tx.skb->len, DMA_TO_DEVICE);
		dev->stats.tx_packets++;			//update the counter for ifconfig
		dev->stats.tx_bytes += lp->tx.skb->len;	//update bcnt

		dev_kfree_skb_irq(lp->tx.skb);	//free the skb
		lp->tx.skb = NULL;	//reinit
		lp->tx.phys = 0;	//reinit local vars

		netif_trans_update(dev);	//linux kernel specific
		netif_wake_queue(dev);		//wake up the queue
...
//kernel calls this first when it has a packet to transmit
...
static int lmac2_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	writel((uint32_t)lp->tx.phys,
		lp->base_addr + DMA_M2S_OFFSET + DMA_R_ADDR);		//lower 32b
	writel(lp->tx.phys >> 32,
		lp->base_addr + DMA_M2S_OFFSET + DMA_R_ADDR_MSB);	//upper 32b
	writel(len,
		lp->base_addr + DMA_M2S_OFFSET + DMA_R_LENGTH);		//length in byte and cause DMA to start
	lp->tx.in_flight = true;

	dev->stats.tx_bytes += len;		//update counter for ifconfig
	netif_stop_queue(dev);			//stop Q since only handle 1 pkt at a time

	spin_unlock_irqrestore(&lp->tx.lock, flags);	//semaphore to process 1 pkt at a time
	return 0;
}

//calls by the kernel when: ifconfig up
//prepare LMAC for RX pkt
...
//	ndev->flags &= ~IFF_MULTICAST;
	...
	dev_info(dev,
		 "Lewiz at 0x%08X mapped to 0x%08lX, tx-irq=%d rx-irq=%d\n",
